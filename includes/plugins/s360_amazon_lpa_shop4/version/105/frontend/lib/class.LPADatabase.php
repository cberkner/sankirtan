...
class LPADatabase {
     ...
     * @return an array of orders found
     */
        ...
        if (!empty($result)) {
            foreach ($result as &$res) {
                ...
                if ($bestellung) {
                    ...
                    Jtllog::writeLog('LPA: LPADatabase->getOrdersPaged() : $bestellung = NULL (kBestellung=' . intval($res->kBestellung) . ')', JTLLOG_LEVEL_DEBUG);
                    $res->bestellung = NULL;
                }
            }
            return $result;
    ...
    public function getAuthorizationsForOrder($orid) {
        $sql = "SELECT * FROM " . S360_LPA_TABLE_AUTHORIZATION . " WHERE cOrderReferenceId = '{$orid}'";
        $result = Shop::DB()->query($sql, 2);
        if ($result) {
            return $result;
        } else {
    ...
    public function getAuthorizationsForOrders($idArray) {
        ...
        $ids = implode(',', array_map(function($a) {return "'" . $a . "'";}, $idArray));
