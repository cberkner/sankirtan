...
class EvoEditor
{
    ...
    private function save()
    {
        if (isset($_REQUEST['data']['content']) && isset($_REQUEST['data']['file'])) {
            ...
            $source = (strpos($_REQUEST['data']['file'], PFAD_ROOT) !== false) ? $_REQUEST['data']['file'] : (realpath($this->path.'/../../../').'/less/'.$_REQUEST['data']['file']);
    ...
    {
        $filePath = (strpos($_REQUEST['data']['file'], PFAD_ROOT) === false) ? ($this->themesPath.$_REQUEST['theme'].'/less/'.$_REQUEST['data']['file']) : $_REQUEST['data']['file'];
        $return['fn'] = 'openFile';
    ...
    public function compile($theme = null, $template = null)
    {
        ...
        $directory = ($template === null) ? $this->themesPath.$theme : PFAD_ROOT.PFAD_TEMPLATES.$template.'/themes/'.$theme;
     ...
     *
     * @param string $type
     *                     message class (danger, success, info)
     * @param string $msg
     *                     message text
     *
     * @return array json output
