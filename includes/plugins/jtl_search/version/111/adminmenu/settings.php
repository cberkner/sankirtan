...
require_once PFAD_ROOT . PFAD_PLUGIN . $oPlugin->cVerzeichnis . '/' .
    PFAD_PLUGIN_VERSION . $oPlugin->nVersion . '/includes/defines_inc.php';
...
$oSettings_arr->jtlsearch_suggest_align    = (isset($oPlugin->oPluginEinstellungAssoc_arr['jtlsearch_suggest_align']))
    ? $oPlugin->oPluginEinstellungAssoc_arr['jtlsearch_suggest_align'] :
    'left';
$oSettings_arr->jtlsearch_export_languages = [];
$oSettings_arr->jtlsearch_branding         = (isset($oPlugin->oPluginEinstellungAssoc_arr['jtlsearch_branding']))
    ? $oPlugin->oPluginEinstellungAssoc_arr['jtlsearch_branding']
    : '1';
//load available languages
$oLanguage_arr    = Shop::DB()->query("
    SELECT cISO, cNameDeutsch, cShopStandard 
          FROM tsprache 
          ORDER BY cShopStandard DESC, cNameDeutsch", 2
);
$cLanguageISO_arr = [];
...
if (isset($_POST['stepPlugin']) && $_POST['stepPlugin'] == $stepPlugin) {
    ...
    Shop::DB()->update(
        ...
        ['cName', 'kPlugin'],
        ['jtlsearch_suggest_align', $oPlugin->kPlugin],
        (object)['cWert' => $oPlugin->oPluginEinstellungAssoc_arr['jtlsearch_suggest_align']]
    );
    ...
    switch ($_POST['jtlsearch_branding']) {
        case '0':
            $oPlugin->oPluginEinstellungAssoc_arr['jtlsearch_branding'] = '0';
            break;
        case '1':
        default:
            $oPlugin->oPluginEinstellungAssoc_arr['jtlsearch_branding'] = '1';
            break;
    }
    Shop::DB()->update(
        'tplugineinstellungen',
        ['cName', 'kPlugin'],
        ['jtlsearch_branding', $oPlugin->kPlugin],
        (object)['cWert' => $oPlugin->oPluginEinstellungAssoc_arr['jtlsearch_branding']]
    );
    $oSettings_arr->jtlsearch_branding = $oPlugin->oPluginEinstellungAssoc_arr['jtlsearch_branding'];
...
Shop::Smarty()->assign('oLanguage_arr', $oLanguage_arr)
    ...
    ->assign('URL_ADMINMENU', Shop::getURL() . '/' . PFAD_PLUGIN . $oPlugin->cVerzeichnis . '/' .
