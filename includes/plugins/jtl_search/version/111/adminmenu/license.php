...
if (isset($oServerSettings->cProjectId) && strlen($oServerSettings->cProjectId) > 0 &&
    ...
    $oForm->addElement('kPlugin', 'hidden', '', ['value' => $oPlugin->kPlugin]);
    $oForm->addElement('cPluginTab', 'hidden', '', ['value' => 'Lizenz']);
    $oForm->addElement('stepPlugin', 'hidden', '', ['value' => $stepPlugin]);

    $oForm->addElement('cCode', 'textarea', 'Lizenzschlüssel', ['style' => 'width: 500px; height: 120px;', 'value' => $cLicenseKey, 'id' => 'cCode', 'class' => 'form-control']);
    $oForm->addElement('btn_serverinfo', 'submit', '', ['value' => 'Lizenz speichern', 'class' => 'btn btn-primary button orange']);
        ...
        if ($oForm->isValid()) {
            if ($cLicenseKey != $_POST['cCode']) {
                ...
                if (is_array($cData_arr) && count($cData_arr) === 2) {
                    ...
                    $oSecurity->setParam_arr(['getsearchserver']);
                ...
                $oDebugger->doDebug(__FILE__ . ': Es muss nichts geändert werden da die Keys dieselben sind.', JTLSEARCH_DEBUGGING_LEVEL_DEBUG);
            }
        }
    }
