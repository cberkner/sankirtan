...
class JTLSearchExportQueue
{
     ...
     * @var array
     */
    private $nCount_arr = ['category' => 0, 'manufacturer' => 0, 'product' => 0];
    /**
     * @access private
     * @var array
     */
    ...
    public function __construct(IDebugger $oDebugger, $nExportMethod)
    {
        ...
        if (is_numeric($nExportMethod) && $nExportMethod > 0) {
            $this->oDebugger->doDebug(__FILE__ . ':' . __CLASS__ . '->' . __METHOD__ .
                '; ExportQueue mit $nExportMethod = ' . $nExportMethod . ' im Konstruktor laden');
            $this->loadFromDB($nExportMethod);
        } else {
            $this->oDebugger->doDebug(__FILE__ . ':' . __CLASS__ . '->' . __METHOD__ .
                '; $nExportMethod muss eine Zahl sein und größer gleich 1 ($nExportMethod = ' . $nExportMethod . ')');
            die('JTL-Search Fehler beim Datenexport: ' .
                'nExportMethod hat einen invaliden Wert ($nExportMethod). Es wird ein ganzzahliger Wert >= 1 erwartet.');
        }
    }
    ...
    private function loadFromDB($nExportMethod)
    {
        $oObj = Shop::DB()->query(
            "SELECT *
                 ...
                 WHERE nExportMethod = " . (int)$nExportMethod . " 
                 AND bFinished = 0 
                 AND bLocked = 0", 1
        );
        ...
        if (isset($oObj->kExportqueue) && $oObj->kExportqueue > 0) {
            ...
            $this->update();
            $this->oDebugger->doDebug(__FILE__ . ':' . __CLASS__ . '->' . __METHOD__ .
                '; JTLSearchExportQueue mit der ID ' . $this->kExportqueue .
                ' wurde geladen (nLimitn = ' . $this->nLimitN . ', nLimitM = ' .
                $this->nLimitM . ', nExportMethod = ' . $this->nExportMethod . ')');
        } else {
            $this->oDebugger->doDebug(__FILE__ . ':' . __CLASS__ . '->' . __METHOD__ .
                '; Keine JTLSearchExportQueue fuer $nExportMethod = ' . $nExportMethod . ' vorhanden.');
        }
    ...
    public function setCount($cKey, $nValue)
    {
        if (is_string($cKey) && isset($this->nCount_arr[$cKey]) && is_int($nValue)) {
            ...
            if (!isset($this->nCount_arr[$cKey])) {
                $this->oDebugger->doDebug(__FILE__ . ':' . __CLASS__ . '->' . __METHOD__ .
                    '; Es gibt kein $nCount_arr[' . $cKey . '].');
            } else {
                $this->oDebugger->doDebug(__FILE__ . ':' . __CLASS__ . '->' . __METHOD__ .
                    '; Parameter falsch ($cKey = ' . $cKey . ', $nValue = ' . $nValue . ').');
            }
        }
    ...
    private function loadExportObjects()
    {
        if ($this->nExportMethod == 3) {
            $oExport_arr = Shop::DB()->query("
                SELECT kId, eDocumentType 
                    FROM tjtlsearchdeltaexport
                    WHERE bDelete = 0 
                    LIMIT 0, " . JTLSEARCH_LIMIT_N_METHOD_3, 2
            );
            ...
            foreach ($this->nCount_arr as $cKey => $nCount) {
                ...
                if ($nExportRun > 0) {
                    $cClass   = $cKey . 'Data';
                    $oRes_arr = call_user_func([$cClass, 'getItemKeys'], $nExported, $nExportRun);
                    foreach ($oRes_arr as $oRes) {
                        $this->xExportObject_arr[] = [$cKey, $oRes->kItem];
                    }
                } else {
...
//            $this->oDebugger->doDebug(__FILE__.':'.__CLASS__.'->'.__METHOD__.'; '.  print_r($oObj, true));
    ...
    private function update()
    {
        ...
        return Shop::DB()->update('tjtlsearchexportqueue', 'kExportqueue', $this->kExportqueue, $upd);
    }
    ...
    {
        return Shop::DB()->deleteRow('tjtlsearchexportqueue', 'kExportqueue', $this->kExportqueue);
    }
    ...
    {
        $this->kExportqueue = (int)$kExportqueue;
    ...
    {
        $this->nExportMethod = (int)$nExportMethod;
