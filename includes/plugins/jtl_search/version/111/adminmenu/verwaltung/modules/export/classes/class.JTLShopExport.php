...
class JTLShopExport extends Export
{
    ...
    protected function lastRun()
    {
        ...
        if ($this->oJTLSearchExportQueue->getExportMethod() == 3) {
            $oCountExport_arr = Shop::DB()->selectAll('tjtlsearchdeltaexport', 'bDelete', 1);
        ...
        if ($oReturnObj->nExported > 0) {
            ...
            if($this->zipFile()) {
                ...
                $this->oDebugger->doDebug(__FILE__ . ':' . __CLASS__ . '->' . __METHOD__ .
                    '; $oReturnObj = ' . print_r($oReturnObj, true));
            } else {
                $this->oDebugger->doDebug(__FILE__ . ':' . __CLASS__ . '->' . __METHOD__ .
                    '; Daten konnten nicht gezippt werden.', JTLSEARCH_DEBUGGING_LEVEL_CRIT);
                $oReturnObj->nServerResponse = 0;
                $oReturnObj->cMessage = "Daten konnten nicht gezippt werden.";
            }
        } else {
            $this->oDebugger->doDebug(__FILE__ . ':' . __CLASS__ . '->' . __METHOD__ .
                '; Keine Daten exportiert (' . $oReturnObj->nExported . ').');
            $oReturnObj->nServerResponse = 0;
        }
        switch ($this->oJTLSearchExportQueue->getExportMethod()) {
            //Export über Cron
            ...
            case 3:
                ...
                if ($nDeletedRows != $oReturnObj->nCountAll) {
                    $this->oDebugger->doDebug(__FILE__ . ':' . __CLASS__ . '->' . __METHOD__ . '; ' . $nDeletedRows .
                        ' Zeilen aus tjtlsearchdeltaexport gelöscht, ' . $oReturnObj->nCountAll . 'Zeilen exportiert.');
                }
    ...
    protected function sendFileToImportQueue()
    {
        ...
        $this->oDebugger->doDebug(__FILE__ . ':' . __CLASS__ . '->' . __METHOD__ .
            '; File in die Importqueue des Suchservers schicken.');
        ...
        if(isset($oServerSettings) && $oServerSettings !== false && is_object($oServerSettings)) {
            ...
            $this->oDebugger->doDebug(__FILE__ . ':' . __CLASS__ . '->' . __METHOD__ .
                '; $xData_arr = ' . print_r($xData_arr, true));
