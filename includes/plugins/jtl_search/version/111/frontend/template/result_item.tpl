			...
			{if $oItem->cImageUrl|@strlen > 0}
				<div class="jtl-search-item article_wrapper clearall">
					...
					<div class="article_info">
						{$oItem->cName|regex_replace:"/({$cSearch|escape:'url'})/i":"<span class='jtl_match'>\$1</span>"}
					</div>
				</div>
			{else}
				{$oItem->cName|regex_replace:"/({$cSearch|escape:'url'})/i":"<span class='jtl_match'>\$1</span>"} {if $oItem->nCount > 0}
